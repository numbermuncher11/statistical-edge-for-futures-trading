...
```{r}
holidays<-unique(lasttick.dt[lasttick < 42300]$Date)
...
get.holidays<-function(contract){
  filename <- paste('txt/',contract,'.txt',sep='')
  dates<- readRDS(paste('dates/',contract,'.rds',sep=''))
  tickdata.dt <- loadtxtfile(filename)[Date %in% dates]
...
for(x in c(ZN.contracts,'ZNH19')){
  holiday.dates<-c(holiday.dates,get.holidays(x))
}
...
print(b)
```
Looks like there's 96.7%  probability that the ZN price in the RTH session breaks the European session high or low. The 95% confidence interval is (95.3%, 98.2%) That's important to know. How about the TN or ZB? Do they behave in a similar manner?
...
Here, the p-values for all three instruments is quite high. They don't seem to favor breaking the session highs or lows. 
...
89.3%. That's a pretty good correlation. There's a small error in our logic though. We are not taking into account time of day. We are just counting if they break on the same day. For instance, if ZN breaks first before the ZB, our test wouldn't know that. We can't tell the future so we shouldn't include that in the case. We should only include cases where the ZB breaks first since we want to see if ZN follows. Let's redo the test.

```{r}
daily.combined[BrkNH.ZB==TRUE & (BrkNH.ZN==FALSE | (BrkNH.Time.ZB < BrkNH.Time.ZN)),prob(BrkNH.ZN)]
```

Much better. Here we only included cases where either the ZN didn't break the high or it broke after the ZB. 81.5% is still very high, though not as good as 89.5%.
...
The average is around 81.5%. At the very least, we should expect a 76.2% probability that the ZN will follow the ZB in breaking the highs. A pretty good correlation.
...
```
83.9%. At the very least, we should expect a 78.8% chance that ZB follows the ZN. Much lower than if ZB is the leader. Does this mean that ZB is more of a leader for the ZN? Let's check p values to make sure if our relationship is statistically significant.
...
kable_styling(k,bootstrap_options = 'striped',full_width=F,position = 'left')
```
Great! We have some encouraging results. Our strategy is profitable across 3 different instruments across all the various combination of correlations. Some correlations seem to work better than others however. The 'ZB following ZN' had the highest profit factor (1.6), followed by the 'TN following ZB' (1.5), 'TN following ZN' (1.4), 'ZN following ZB' (1.4).  The lowest profit factor was the 'ZN following TN' strategy(PF=1.1). In general, we see that if the ZN leads, the strategies tend to work better than if the ZN follows. This makes sense as the ZN is the most heavily traded contract. When the ZN moves, it can be considered more significant because more volume is being exchanged. The other contracts have much lower traded volume so the moves might not be considered as significant.

If we break it down further to long vs short strategy, we see that the long side is much more profitable than the short side. In some cases, the short side produced negative returns, like the 'ZN following TN'strategy. The most profitable short selling strategy, 'TN following ZB', only had a 1.5 profit factor, much smaller than the top long trading strategy, 'ZB following ZN' (PF=1.9).
...
print(pval)
```
The p-value for the difference of the profit factors between the buy side and sell side is 0.178. While this may not satisfy the scientific standard of 0.05, we may make a weak conclusion that there is a difference between the buy and sell side profit factors. For our trading, we will only do the buy side as it seems to be more profitable across various different instruments.
...
We see that the bottom -2.5% percentile max drawdown is -65 ticks with an average of -38 ticks. What this means is that if our system suffers a -65 ticks drawdown, we can consider it to be no longer valid for the current market conditions.
